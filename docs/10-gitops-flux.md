...
### Step 2: Cloud Shell에서 본인 fork로 remote 변경

워크샵 초반(`01-prerequisites`)에 이미 `bbiggum/azure-aks-workshop`을 clone 했다면, **remote URL만 본인 fork로 교체**하세요. 처음부터 clone 한다면 본인 fork URL로 받으면 됩니다.

```bash
# 본인 GitHub 사용자명을 환경 변수로 (이후 단계에서 계속 사용)
export GITHUB_USERNAME="<YOUR_GITHUB_USERNAME>"

cd ~/azure-aks-workshop

# 기존 remote를 본인 fork로 변경
git remote set-url origin https://github.com/$GITHUB_USERNAME/azure-aks-workshop.git

# 변경 확인 — origin 이 본인 계정 URL이어야 함
git remote -v
```
...
- [ ] `git remote -v` → origin URL이 `https://github.com/<본인>/azure-aks-workshop.git`
...
source-controller-54f4cc9f5b-rj8lw         0/1     Running   1          110m
```
...
workshop-gitops-store-front   6m34s   False   Source artifact not found, retrying in 30s
```
